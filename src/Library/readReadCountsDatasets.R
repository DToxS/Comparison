...
readReadCountsDatasets <- function(read_counts_files, generateReadCountsColumnNames=NULL, split_char=NULL, field_names=NULL, field_types=c(Species="character", Subject="character", State="character", Culture="character", Replicate="character", Measure="character", Sample="character", Time="character"), species_default=species_default, subject_default=subject_default, state_default=state_default, culture_default=culture_default, measure_default=measure_default, time_default=time_default, row_name_column=NULL, read_counts_columns=NULL, read_counts_skip=0, exprt_design_files=NULL, field_names_positions=NULL, exprt_design_skip=0, legal_chars=NULL, repl_chars=NULL, compress_space=TRUE, func_dir=NULL)
{
    ...
    for(idx in 1:length(read_counts_files))
    {
        ...
        if(is.null(exprt_design_files))
        {
            ...
            exprt_design <- generateExperimentDesignTable(read_counts_column_names=read_counts_cols_prep, split_char=split_char, field_names=field_names, field_types=field_types, species_default=species_default, subject_default=subject_default, state_default=state_default, culture_default=culture_default, replicate_default=replicate_default, measure_default=measure_default, sample_default=1:length(read_counts_cols_prep), time_default=time_default, legal_chars=legal_chars, repl_chars=repl_chars, compress_space=compress_space, func_dir=func_dir)
        }
        else
        {
            ...
            exprt_design <- convertExperimentDesignTable(exprt_design=exprt_design_dataset, field_names_positions=field_names_positions, field_types=field_types, species_default=species_default, subject_default=subject_default, state_default=state_default, culture_default=culture_default, replicate_default=replicate_default, measure_default=measure_default, sample_default=1:nrow(exprt_design_dataset), time_default=time_default, legal_chars=legal_chars, repl_chars=repl_chars, compress_space=compress_space, func_dir=func_dir)
        }
