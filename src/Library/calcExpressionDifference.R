...
calcExpressionDifference <- function(read_counts, exprt_design, drug_groups, bio_spec, time_length, results_dir, fdrs=c(0.05), state_name_ctrl="CTRL", min_samples=3, rep_req="all", dispersion="auto", n_top=100, deg_only=FALSE, norm_base=1e+6, remove_small=TRUE, plot_bcv=TRUE, plot_smear=TRUE, pt_chars=c(16,19), read_counts_title="ReadCounts", deg_title="DEG", top_title="TOP", exp_configs_title="ExpConfigs", calc_title="Calc", plot_title="Plots", verbose1=FALSE, verbose2=FALSE, func_dir=NULL)
{
    ...
    for(fdr in fdrs)
    {
        ...
        for(drug_group_name in names(drug_groups))
        {
            ...
            for(cell_line in cell_lines)
            {
                ...
                for(plate in plates)
                {
                    ...
                    stats_datasets <- recordGeneExpDiffs(deg_read_counts=deg_read_counts_plate, targets=targets_compare_group_cell_plate, state_names=state_names_compare, read_counts=read_counts_group, cell_line=cell_line, plate=plate, n_top=n_top, read_counts_file=read_counts_file, exp_configs_file=exp_configs_file, top_stats_file=top_stats_file, deg_stats_file=deg_stats_file, verbose=verbose1, func_dir=func_dir)
        ...
        if(!is.null(deg_counts_sum)) deg_counts_sums <- rbind(deg_counts_sums, data.frame(FDR=rep(fdr,nrow(deg_counts_sum)), deg_counts_sum, stringsAsFactors=FALSE))
    }

    # Save DEG statistics at all FDRs.
    if(!is.null(deg_counts_sums) && !is.null(deg_title))
    {
