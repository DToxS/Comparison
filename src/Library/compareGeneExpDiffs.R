...
compareGeneExpDiffs <- function(read_counts, group_names, group_pairs, dispersion="auto", min_rep=3, rep_req="all", fdr=0.05, deg_only=TRUE, normalize=TRUE, norm_base=1e+6, remove_small=TRUE, exp_info=NULL, deg_read_counts_file=NULL, plot_bcv=FALSE, plot_smear=FALSE, deg_plots_file=NULL, pt_chars=c(16,19), r_compat=TRUE, verbose=FALSE, func_dir=NULL)
{
    ...
    for(pair_idx in 1:nrow(group_pairs))
    {
        ...
        deg_read_counts_list[[paste(group_pair[1],group_pair[2],sep=".")]] <- deg_counts_stats
    }
