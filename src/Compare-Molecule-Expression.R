...
getConfigFile <- function(arg_pos, invoke_method="Rscript", default_file=NULL)
{
    ...
    if(invoke_method == "Rscript")
    {
        ...
        cmd_args <- commandArgs(TRUE)
        if(length(cmd_args) > arg_pos-1) config_file <- cmd_args[arg_pos]
        else
        {
...
getRepoDir <- function(arg_pos, invoke_method="Rscript", default_dir=getwd())
{
    ...
    if(invoke_method == "Rscript")
    {
        ...
        cmd_args <- commandArgs(TRUE)
        if(length(cmd_args) > arg_pos-1) repo_dir <- cmd_args[arg_pos]
        else
        {
...
getFuncDir <- function(arg_pos, invoke_method="Rscript", default_dir=getwd())
{
    # invoke_method: "Rscript" or "R".
    ...
    if(invoke_method == "Rscript")
    {
        ...
        cmd_args <- commandArgs(TRUE)
        if(length(cmd_args) > arg_pos-1) func_dir <- cmd_args[arg_pos]
        else
        {
...
config_file <- getConfigFile(1, invoke_method=invoke_method, default_file=default_config_file)
...
repo_dir <- getRepoDir(2, invoke_method=invoke_method, default_dir=default_repo_dir)
...
func_dir <- getFuncDir(3, invoke_method=invoke_method, default_dir=default_func_dir)
